# -*- coding: utf-8 -*-
"""
Created on Fri May 11 23:11:36 2018
...
def getandplot():
    heig = int(text1.get())
    leng = int(text2.get())
    if leng/heig<10 or leng/heig>15:
        root1 = Tk()
...
text1 = Entry(root)
text1.pack()
Label(root,text = '输入梁长度(cm),建议为梁高的10-18倍').pack()
text2 = Entry(root)
text2.pack()
button1 =Button(root,text = '确定并显示',command = getandplot).pack()

Label(root,text = '输入荷载大小(KN)，建议值为20-80').pack()
text3 = Entry(root)
text3.pack()
button2  = Button(root,text = '简支梁（跨中荷载）变形图',command = jianzhi).pack()
button3 = Button(root,text = '悬臂梁（悬臂端荷载）变形图',command = xuanbi).pack()
root.mainloop()
